        ...
        if self.distill_cfg is not None:
            for item_loc in distill_cfg:
                for item_loss in item_loc.methods:
                    ...
                    if use_this and loss_name != "loss_fmkd":
...
@MODELS.register_module()
class FMKTClassificationDistiller(ClassificationDistiller):
    ...
    def __init__(self,
                 ...
                 data_preprocessor: Optional[dict] = None,
                 init_cfg: Optional[dict] = None):
        super(FMKTClassificationDistiller, self).__init__(
            teacher_cfg,
            student_cfg,
            is_vit,
            use_logit,
        ...
        if self.distill_cfg is not None:
            for item_loc in distill_cfg:
                for item_loss in item_loc.methods:
                    ...
                    use_this = item_loss.use_this
                    if use_this and loss_name == "loss_fmkd":
                        self.flow = FlowAlignModule(teacher_channel=item_loss.teacher_channel,
                                                    student_channel=item_loss.student_channel,
                                                    type=item_loss.loss_type,
                                                    teacher_size=1,
                                                    student_size=7,
                                                    sampling=8,
                                                    loss_type=item_loss.flow_loss_type,
                                                    encoder_type=item_loss.encoder_type,
                                                    number=item_loss.number)
                        self.inference_sampling = item_loss.inference_sampling
    ...
    def forward(self,
        ...
        if mode == 'tensor':
            feats = self.student.extract_feat(inputs)
            cls_token = feats[-1][-1]
            return self.flow(cls_token, None, inference_sampling=self.inference_sampling)[
                1] if self.student.with_head else cls_token
        elif mode == 'loss':
            return self.loss(inputs, data_samples)
        elif mode == 'predict':
            feats = self.student.extract_feat(inputs)
            cls_token = feats[-1][-1]
            return self.flow(cls_token, None, inference_sampling=self.inference_sampling)[1]
        else:
            raise RuntimeError(f'Invalid mode "{mode}".')
    ...
    def loss(self, inputs: torch.Tensor,
        ...
        flow_loss, logit_s = self.flow(x[-1][-1], logit_t, target=gt_label, cls_loss=lambda aa,bb: self.student.head.loss_module(aa,bb, avg_factor=aa.size(0)))
        # if self.student.with_head and hasattr(self.student.head, 'pre_logits'):
        #     x = self.student.head.pre_logits(x)
        # if self.is_vit:
        #     logit_s = self.student.head.layers.head(x)
